...
/// 迪杰斯特拉算法
/// </summary>
public class DijstraAlgorithm : IAlgorithm
{
    //各个点到起点的最短路径距离
    ...
    void NavigationStepByStep(SlotController nowSlot)
    {
        ...
        foreach (SlotController slot in neighbors)
        {
            ...
            //计算从当前点到邻居点的距离
            int distance = minDistanceArray[(int)nowSlot.Pos.x][(int)nowSlot.Pos.y] + slot.Weight;
    ...
    SlotController FindMin()
    {
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            ...
            {
                if (minDistanceArray[i][j] < 0)
                {
                    continue;
                }
                if ((minDistance < 0 || minDistanceArray[i][j] < minDistance) && !flagArray[i][j])
                {
                    minDistance = minDistanceArray[i][j];
                    x = i;
                    y = j;
                    continue;
                }
